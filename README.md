...
|---|---|---|---|
| `.scripts/release/check_update_secret_inventory.py` | Value-safe bootstrap/rotation setter plus inventory preflight for the seven update-delivery names and versioned Sparkle generations; setter values arrive only via stdin or owner-only files and are never argv/output | `make updates-secret-inventory`; `make updates-secret-inventory CHECK_GITHUB=1`; or see the runbook for `--set`, `--set-developer-id-from`, and `--set-notary-from` | Exit 0 when the requested storage/preflight action succeeds; exit 1 on invalid input, missing/unexpected names, or unavailable/failed `gh`; no credential-bearing artifacts |
