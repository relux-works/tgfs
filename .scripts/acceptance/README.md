...
| `run_installed_live_content.py` | Bounded installed authorized-profile date-first enumeration, one fresh placeholder hydration, generated-cache accounting, and relaunch comparison | **Yes, on the authorized local macOS profile.** The before query excludes generated document sets above 8MiB, then gives the selected foreground attachment a killable 60-second read and verified cache publication before three killable 10-second cache+Finder exact-byte pairs. The 90-second read envelope fits inside the real 118-second worker budget; the unchanged 120-second aggregate retains two seconds for TERM/KILL/reap and pipe cleanup. Read children stay in the worker process group, and timeout/read failures remain fixed fail-closed categories. Schema v25 supplies the sort-free small-attachment catalog; selection examines at most 20 candidates with 500ms child probes and fixed aggregate state counters, including missing, platform-error, timeout, and identity/path-mismatch outcomes. A missing catalog item is resolved once by exact stable ItemId through the installed companion's pre-AppKit headless File Provider command, which validates through the core codec and round-trips the returned URL to the same provider identity before reporting success; then the harness independently requires the expected path to appear dataless. The identifier travels on stdin and never enters argv or public evidence. Public evidence excludes identifiers, paths, names, and content; it includes only the aggregate selected generated bytes and fixed 8MiB limit, while bulk identity/cursor keys remain in an indexed SQLite sidecar beside the temporary private state. |
...
before its single bounded attachment open. It selects only a chat/month whose
three generated documents total at most 8MiB, observes foreground cache
publication first, and then opens one current nonempty
`Messages.md`, `Messages.ndjson`, and hidden `.chat.json`, compares their exact bytes
with the verified generated cache in three bounded cache+Finder child reads,
validates MIME/size/version/date facts, and
