...
func TestSetupLocalCreatesInstalledRuntime(t *testing.T) {
	...
	assertSymlink(t, filepath.Join(project, ".codex", "fast.config.toml"), filepath.Join(project, ".agents", ".configs", "fast.config.toml"))
	assertSymlink(t, filepath.Join(project, ".codex", "local.model-catalog.json"), filepath.Join(project, ".agents", ".configs", "local.model-catalog.json"))
...
func seedSourceRepo(t *testing.T) string {
	...
	mustWrite(t, filepath.Join(root, ".configs", "fast.config.toml"), "model = \"gpt-5.4\"")
	mustWrite(t, filepath.Join(root, ".configs", "local.model-catalog.json"), "{\"models\":[]}")
