...
let qaFaultControlEnabled =
    ProcessInfo.processInfo.environment["GRAMDRIVE_QA_FAULT_CONTROL"] == "1"
let qaFaultSecret = ProcessInfo.processInfo.environment["GRAMDRIVE_QA_FAULT_SECRET"]
if qaFaultControlEnabled {
    guard let qaFaultSecret,
        qaFaultSecret.count == 64,
        qaFaultSecret.range(of: "^[0-9a-f]{64}$", options: .regularExpression) != nil
    else {
        fatalError(
            "GRAMDRIVE_QA_FAULT_CONTROL=1 requires a 32-byte lowercase hex "
                + "GRAMDRIVE_QA_FAULT_SECRET")
    }
}
let qaFaultSwiftSettings: [SwiftSetting] = qaFaultControlEnabled
    ? [.define("GRAMDRIVE_QA_FAULT_CONTROL")]
    : []
let qaAgentDependencies: [Target.Dependency] = qaFaultControlEnabled
    ? [.target(name: "GramDriveQAFaultSecret")]
    : []
...
var packageTargets: [Target] = [
        ...
        .target(
            ...
            swiftSettings: qaFaultSwiftSettings
        ),
        .target(
