...
@Suite struct AgentLifecycleTests {
    ...
    @Test func startReachesRunningWithStateOpenAndHealthServing() async throws {
        try await withTemporaryDirectoryAsync { root in
            ...
            let healthListening = try #require(health.recentEvents.firstIndex(of: "health-listening"))
            let stateOpening = try #require(health.recentEvents.firstIndex(of: "state-opening"))
            let stateOpened = try #require(health.recentEvents.firstIndex(of: "state-opened"))
            #expect(healthListening < stateOpening)
            #expect(stateOpening < stateOpened)
