...
///   repair, account removal, and account-scoped content policy. The health
///   socket is read-only by design (`AgentHealthServer`: "control operations
///   stay where they belong … not an IPC verb"). The live backend uses the
///   typed control channel and reports ``ControlChannelUnavailable`` when it
///   cannot establish or retain that channel.
public protocol CompanionBackend: Sendable {
  ...
  /// Reads one account's committed retention/Archive state.
  func fetchContentPolicy(accountId: Int64) async
    -> PolicyOutcome<ControlContentPolicyStatus>
  /// Requests a retention transition. Audit-to-Mirror carries the exact
  /// phrase the user typed; this method never synthesizes confirmation.
  func setRetention(
    accountId: Int64,
    target: ControlRetentionMode,
    typedConfirmation: String?
  ) async -> PolicyOutcome<ControlRetentionTransition>
  /// Toggles the independent per-account Archive Mode setting.
  func setArchiveMode(
    accountId: Int64,
    enabled: Bool
  ) async -> PolicyOutcome<ControlArchiveModeTransition>
  /// Resumes a durable physical-file purge after interruption.
  func resumeRetentionPurge(accountId: Int64) async
    -> PolicyOutcome<ControlRetentionPurgeResume>
}

/// Narrow optional capability used by the AppKit termination gate. Keeping it
/// separate from the UI backend avoids making previews invent a process drain.
public protocol CompanionTerminationPreparing: Sendable {
...
/// Typed result for a policy read or command. The value is the engine's
/// returned state/report; a dropped channel never guesses whether a command
/// committed.
public enum PolicyOutcome<Value: Equatable & Sendable>: Equatable, Sendable {
  case value(Value)
