...
public final class CompanionViewModel {
  ...
  public func startAgentSession() async {
    for attempt in 0..<Self.startupReadinessAttempts {
      guard let readout = await refreshHealthAndAuthorization() else { return }
      if Self.hasAuthorizedAccount(in: readout) {
        await onboarding.prepareFileProviderDomain()
        return
      }
      guard attempt + 1 < Self.startupReadinessAttempts,
        Self.isStartupReadinessPending(readout)
      else { return }
      do {
        try await Task.sleep(for: Self.startupReadinessRetryInterval)
      } catch {
        return
      }
    }
  ...
  /// A cold installed agent can answer health before retained accounts and the
  /// Finder hierarchy have converged. Domain reconciliation installs the live
  /// change relays, so treating that transitional snapshot as settled leaves
  /// previously materialized generated bytes outside later publication.
  private nonisolated static let startupReadinessAttempts = 40
  private nonisolated static let startupReadinessRetryInterval = Duration.milliseconds(250)

  private nonisolated static func isStartupReadinessPending(_ readout: HealthReadout) -> Bool {
    guard case let .running(snapshot) = readout else { return false }
    return snapshot.state == .launching || snapshot.state == .recovering
