...
| Type | Owns |
|---|---|
| `AgentLifecycle` | The coordinator process's lifecycle: single-instance guard first, then shared state as `.coordinator` with corruption recovery (quarantine + one retry), the `DriveCore` handle, the health endpoint, the hydration endpoint (when a hydrator is wired), power observation. A structurally valid durable namespace-readiness record seeds Finder usability across restart while retryable owner recovery continues; new/incomplete profiles stay preparing. Native authorization is reported independently as soon as its boundary completes, so a later retryable snapshot-membership failure cannot erase the observed authorized state. `shutdown(reason:)` drains before tearing anything down |
