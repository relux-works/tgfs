...
| `InstalledPlaceholderResolutionCommand` | Privacy-safe installed acceptance seam hosted by the companion bundle that owns the embedded extension. `GramDrive --acceptance-resolve-placeholder` accepts one opaque ItemId on stdin, validates it through the authoritative Rust codec, resolves only its user-visible File Provider URL, and round-trips that URL to the same identifier and domain before emitting a fixed label. It requests no content; the caller owns the hard process bound and separately proves the expected path is still dataless before reading it. |
