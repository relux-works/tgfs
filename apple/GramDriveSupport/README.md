...
| `ChangeSignalRelay` | The doorbell→Finder bridge (PLAT-MAC-004 change signaling): observes GramDrive's Darwin doorbell, probes `dataVersion()`, and publishes working-set/root/changed-parent signals only when the probe moved. File Provider's materialized-items enumerator reports materialized containers, so changed live generated documents retain their parent identifiers and are selected for eviction when that parent is materialized. This bounds lifetime-journal replay to Finder-visible subtrees while invalidating any stale generated child before its change signal. Materialized-set enumeration is cancelled after eight seconds; a failed or partial read remains an error rather than becoming an empty-set success. Doorbells and agent-replacement requests share one coalescing dispatch lane, and version/journal checkpoints advance only after successful completion, so a newer request cannot discard an older failed generated-eviction range. Hosted by the domain-registering process (the extension only runs while requests are in flight); cold installed startup waits boundedly for explicit retained-state preparation before installing the relay, and probe-on-start covers rings missed while not running |
