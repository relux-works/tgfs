...
| `ChangeSignalRelay` | The doorbell→Finder bridge (PLAT-MAC-004 change signaling): observes GramDrive's Darwin doorbell, probes `dataVersion()`, and publishes working-set/root/changed-parent signals only when the probe moved. Initial publication merges the account-scoped current live-generated snapshot with journal replay, covering upgraded rows that predate the journal; later passes remain journal-delta-only. File Provider's materialized-items enumerator reports materialized containers, so generated documents retain their parent identifiers and are selected for eviction only when that parent is materialized. This bounds initial snapshot and lifetime-journal replay to Finder-visible subtrees while invalidating stale generated children before change signals. Generated eviction is issued in two-item utility-QoS turns separated by a short scheduling gap, allowing unrelated foreground `fetchContents` callbacks to enter File Provider while a retained-profile replay remains active. Evictions still complete before matching signals, and ordinary attachments, tombstones, and unrelated parents are excluded. Materialized-set enumeration is cancelled after eight seconds; a failed or partial read remains an error rather than becoming an empty-set success. Doorbells and agent-replacement requests share one coalescing dispatch lane, and version/journal checkpoints advance only after successful completion, so a newer request cannot discard an older failed generated-eviction range. Hosted by the domain-registering process (the extension only runs while requests are in flight); cold installed startup waits boundedly for explicit retained-state preparation before installing the relay, and probe-on-start covers rings missed while not running |
